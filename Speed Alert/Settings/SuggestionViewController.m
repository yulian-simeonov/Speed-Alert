...
- (void)viewDidLoad
{
    ...
    [txt_content.layer setBorderWidth:2.0];
...
-(void)OnSubmit
{
    ...
    {
        UIAlertView* alertVw = [[UIAlertView alloc] initWithTitle:@"Warning" message:@"Please put text to submit any feedback." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
        alertVw.tag = 0;
        [alertVw show];
        return;
    }
    ...
    {
        UIAlertView* alertVw = [[UIAlertView alloc] initWithTitle:@"Warning" message:@"Invalid Email Address" delegate:self cancelButtonTitle:@"Try again" otherButtonTitles:nil];
        alertVw.tag = 1;
        [alertVw show];
        return;
    }
...
-(void)SendFeedback
{
    ...
    if ([str_feedback isEqualToString:@"I like it"])
        [self.navigationController pushViewController:[[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"sb_like"] animated:YES];
    else
        [self.navigationController pushViewController:[[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"sb_dislike"] animated:YES];
}
